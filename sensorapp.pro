...
UBUNTU_TRANSLATION_DOMAIN="sensorapp.liu-xiao-guo"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/sensorapp/tests/autopilot/run
aptest.depends  = sub-sensorapp sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/sensorapp/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-sensorapp sub-backend-App
